...
namespace _12thMorning.Models.Queslar.Enchanting {
    public class EnchantServiceInfo {
        ...
        private EnchantServiceInfo(double avg, int level, int cost) {
            ...
            Level = level;
            Cost = cost;
            ActionCount = (int)Math.Round(Math.Sqrt(level) * 4 * 3 / 2);
            var rawValue = ActionCount * .75 * Avg * 2;
        ...
        public static List<EnchantServiceInfo> Build(double avg, int level, int cost) {
            var temp = (int) Math.Round(Math.Sqrt(level) * 4 * 3 / 2);
            var lowerLevel = (int)Math.Ceiling(Math.Pow((temp - .5) / 3 / 4.0 * 2, 2));
            var upperLevel = (int)Math.Ceiling(Math.Pow((temp + .5) / 3 / 4.0 * 2, 2));
